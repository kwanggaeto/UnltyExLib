...
namespace ExLib.Native.WindowsAPI.Utilities
{
    public class XCopy
    {
        #region PInvoke
        ...
        {
            new XCopy().CopyInternal(source, destination, overwrite, nobuffering, progressHandler);
        }

        public static void Copy(string source, string destination, bool overwrite, bool nobuffering,
            EventHandler<ProgressChangedEventArgs> progressHandler, EventHandler completeHandler)
        {
            XCopy copy = new XCopy();
            if (completeHandler != null)
                copy.Completed += completeHandler;

            copy.CopyInternal(source, destination, overwrite, nobuffering, progressHandler);
        }
